 ...
 * CLIProxyAPIPlus qoder-provider branch, mirrored in 9router's
 * shared/qoder/encoding.js).
 *
 * Algorithm:
...
const QODER_S2C = new Int16Array(128).fill(-1);
for (let i = 0; i < 64; i++) {
  QODER_S2C[QODER_STD_ALPHABET.charCodeAt(i)] =
    QODER_CUSTOM_ALPHABET.charCodeAt(i);
}
QODER_S2C["=".charCodeAt(0)] = "$".charCodeAt(0);

/**
 * Encode plaintext bytes/string using Qoder's WAF-bypass scheme.
 * @param plaintext Bytes to encode.
 * @returns encoded string (latin1, safe to send as a binary body).
 */
export function qoderEncodeBody(
