/**
 * Qoder COSY (hybrid RSA+AES+MD5) signing, ported from 9router's
 * shared/qoder/cosy.js (originally CLIProxyAPIPlus qoder-provider branch,
 * internal/auth/qoder/cosy.go).
 *
 ...
 *   - a set of Cosy-* / X-* headers fingerprinting the client (machine id,
 *     IDE version, organization id, etc.)
 *
 ...
 *
 * @param body The exact bytes that will be sent. For GET requests pass an
 *   empty Buffer.
 * @param requestUrl Full request URL (used for sigPath).
 * @param creds Credentials used for signing.
 * @returns Header map ready to merge onto fetch().
 */
